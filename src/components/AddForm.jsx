  ...
  return (
    <AddSection>
      <FileUploadSection>
        ...
        <UploaderWrapper> {previewUrl.map((url,index) => (
          <div key={index}>
             <ImageContainer>
            <ArrayImage src={url} alt={`Preview ${index}`}/>
            <ArrayImageremoveStyleButton onClick={()=>handleRemovePreview(index)} alt={`Preview ${index}`}>❌</ArrayImageremoveStyleButton>
            </ImageContainer>
           </div>
        ))}
        </UploaderWrapper>
      ...
      <InputFieldTitle value={itemTitle} onChange={(event) => setItemTitle(event.target.value)} placeholder="제목"/>
       <CategoryDropdown value={itemcategory} onChange={ChangehandleCategory}>
          ...
          {
             Categories.map(category => (<option key={category.id} value = {category.itemcategory}>
        ...
        <InputFieldComment value={itemInfo} onChange={(event) => setItemInfo(event.target.value)} placeholder="상품 설명"/>
